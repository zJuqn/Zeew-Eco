...
module.exports = {
  ...
  execute: async (client, message, args) => {
    ...
    if (!type)
      return message.channel.send(
        "Debes decir la accion que realizaras. `agregar/remover/reiniciar-tienda`"
      ); //QUE COLOQUE UNA ACCION

    if (type.toLocaleLowerCase() === "agregar") {
      //EMPEZAMOS CON LA ACCION DE AGREGAR UN OBJETO
      const namexd = args[1]; //NOMBRE DE EL OBJETO
      if (!namexd)
        return message.channel.send("Debes decir el nombre de el objeto"); ///SI NO COLOCA QUE RETORNE
      const precio = args[2]; //EL PRECIO DE EL OBJETO
      if (!precio)
        return message.channel.send("Debes decir el precio de el objeto"); //SI NO ES UN NUMERO QUE RETORNE
      const desc = args.slice(3).join(" "); //LA DESC DE EL OBJETO
      if (!desc)
        return message.channel.send("Debes decir la descripcion de el objeto");
    ...
    if (type.toLocaleLowerCase() === "remover") {
      ...
      message.channel
        .send(
          "Para eliminar un item es necesario que veas la tienda y mires el ID de el item, con ese ID lo eliminaras"
        )
        .then((m) => {
    ...
    if (type.toLocaleLowerCase() === "reiniciar") {
      ...
      message.channel
        .send("Estas seguro de eliminar toda la tienda?")
        .then((msg) => {
          //ENVIAMOS MENSAJE CON REACCION
          ...
          msg.awaitReactions((user, reaction) => {
            ...
            if (reaction.emoji.name === "✅") {
              //SI REACCIONA SE ELIMINARA LA TIENDA
              Economia.reiniciar(message.guild.id);
              message.channel.send(
                "La economia a sido eliminada correctamente"
              );
            }
            if (reaction.emoji.name === "❌") {
              //SI REACCIONA SE ELIMINARA EL MENSAJE EN 5 SEGUNDOS
              message.channel.send(
                "El comando a sido cancelado, este mensaje sera eliminado en 5 segundos"
              );
            }
          });
