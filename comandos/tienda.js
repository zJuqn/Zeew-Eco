...
execute: async (client, message, args) => {
    ...
    if(type.toLocaleLowerCase() === 'agregar'){//EMPEZAMOS CON LA ACCION DE AGREGAR UN OBJETO
        const name = args.slice(1).join(' ')//NOMBRE DE EL OBJETO
        if(!name) return message.channel.send("Debes decir el nombre de el objeto")///SI NO COLOCA QUE RETORNE
        const precio = args.slice(3).join(' ')//EL PRECIO DE EL OBJETO
        if(!precio) return message.channel.send("Debes decir el precio de el objeto")
        if(isNaN(precio)) return message.channel.send("Eso no es un numero de precio valido, solo se permiten numeros")//SI NO ES UN NUMERO QUE RETORNE
        const desc = args.slice(4).join(' ')//LA DESC DE EL OBJETO
        if(!desc) return message.channel.send("Debes decir la descripcion de el objeto")
        Economia.agregar(message.guild.id, name, desc, precio)//CREAMOS EL OBJETO
        ...
        message.channel.send("Estas seguro de eliminar toda la tienda?").then(msg => {//ENVIAMOS MENSAJE CON REACCION
            ...
            msg.awaitReactions((user, reaction) => {
                ...
                if(reaction.emoji.name === '✅'){//SI REACCIONA SE ELIMINARA LA TIENDA
                    ...
                    await message.channel.send("La economia a sido eliminada correctamente")
                }
