...
module.exports = {
  ...
  execute: async (client, message, args) => {
    ...
    if (!perms)
      ...
      ); //SI NO TIENE PERMS QUE RETORNE
    let user = message.mentions.users.first() || client.users.resolve(args[0]); //DEFINIMOS A EL USER
    if (!user)
      return message.channel.send(
        "Debes mencionar a un usuario o colocar su ID"
      ); //SI NO MENCIONA QUE RETORNE
    const cantidad = args.slice(1).join(" "); //DEFINIMOS LA CANTIDAD A QUITAR
    if (!cantidad)
      return message.channel.send("Debes decir la cantidad a depositar"); //SI NO COLOCA UNA CANTIDAD QUE RETORNE
    if (isNaN(cantidad))
